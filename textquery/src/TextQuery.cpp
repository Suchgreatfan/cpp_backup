...
std::string TextQuery::queryWordInDict(const std::string &word) const
{
    ...
    for(const auto &pa : dict_)
    {
        int editDistance = stringutil::editDistance(word, pa.first);
...
void TextQuery::readEnDict()
{
    ...
    while(infile >> word >> count)
    {
        ...
        std::pair<std::unordered_map<std::string, int>::iterator, bool> ret = dict_.insert(make_pair(word, count));
